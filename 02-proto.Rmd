...
Notice the similarities already emerging between `retrieveNbytes` and `bytesToInteger`. Perhaps a strategy that would create a function similar to Ruby's `unpack` function would make sense. Curiously, I spent some time researching alternatives to `unpack` and found very few. Even in Python, it apears that there is not a function that works as well as Ruby's implementation.
...
For the timestamp, I validated it with https://www.unixtimestamp.com, which converts the UNIX timestamp into a more readable represenation, `02/22/2014 @ 10:42pm (UTC)`. This date-time is nither wildly in the past nor in the far future.
...
`604.2743` *feels* like a reasonable amount, however I should note that at this point in the exercise that there is very little in the data that will provide feedback on the correctness of the results.

I see three assumtions we might be able to make:

* Timestamps will be in the future of the UNIX epoc. 
...
OK, next up: Loop through all of the records and stop successfully at the end. 
...
In the following new `processRecord` function, we return either a vector of the record values, or an empty vector:
...
In our main function, we capture the vector returned from `processRecord` and add the resuts to a data.frame, `df`. 
...
* Decide if there is a need to address the discrepency in the record count in the header 71, and the actual record count found. I chose to read all 72 records, but perhaps I should have ignored the last one. 
...
* Consider where fractional pennies make sense, if any. Round appropriatly. 
* Build tests that demonstrate and validate the functions are working properly.
* Build in error handling incase the data file or records are malformed.
